...
def train_accuracy(output, target, topk=(1,)):
    ...
    print("shapes",(output.size(), target.size()))
    # Calculate the predicted class indices
    predicted_indices = torch.argmax(output, dim=1)
    print("predicted_indices",predicted_indices.shape)

    target_indices = torch.argmax(target, dim=1)
    print("target_indices",target_indices.shape)
    # Calculate the number of correct predictions
    num_correct = torch.sum(predicted_indices == target_indices).item()
    
    # Calculate the accuracy
    accuracy = 100.0 * (num_correct / target.size(0))
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        acc1 = train_accuracy(output, target, topk=(1,5))
        print("accu1",acc1)
        acc5 = acc1
        top1_m.update(acc1, output.size(0))
        top5_m.update(acc5, output.size(0))
        # top1_m.update(0, output.size(0))
        # top5_m.update(0, output.size(0))
...
#             # Train_acc1, Train_acc5 = accuracy(output, target, topk=(1, 5))
